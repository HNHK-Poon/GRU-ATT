...
parser.add_argument('--sen', metavar='S', default=18, type=int,
                   help='Number of Max Sent (Default: 3)')
parser.add_argument('--wrd', metavar='W', default=40, type=int,
...
def keras_process_amazon(MAX_DOCUMENT_LENGTH,MAX_WORD_LENGTH,MAX_VOCAB):
    data_train = pd.read_csv('../../amazon_review_full_csv/train.csv', sep=',', names = ["label", "text", "text2"])
    print data_train.head(10)
    data_test = pd.read_csv('../../amazon_review_full_csv/test.csv', sep=',',  names = ["label", "text", "text2"])
    print data_test.head(10)
    data_full = pd.concat([data_train, data_test], axis=0)
    data_full = data_full.reset_index(drop=True)
    data_full['text'].fillna('', inplace=True)
    print data_full
    ...
    with open("data/amazon_train.ids", "w") as train_file, open("data/amazon_test.ids", "w") as test_file:
...
def keras_process_AG(MAX_DOCUMENT_LENGTH,MAX_WORD_LENGTH,MAX_VOCAB):
    ...
    data = np.zeros((len(texts), MAX_DOCUMENT_LENGTH, MAX_WORD_LENGTH), dtype='int32')
    with open("data/AG_train.ids", "w") as train_file, open("data/AG_test.ids", "w") as test_file:
        for i, sentences in enumerate(reviews):
            sequence = tokenizer.texts_to_sequences(sentences)
            padded_sequence = pad_sequences(sequence, maxlen=MAX_SENT_LENGTH, dtype='int32',padding='post', truncating='post', value=0.)
            #current_file.write(" ".join([str(id) for id in padded_sequence]) + "\n")
...
#----------------------------------------------------------------------------------------
    ...
    while epoch < num_epochs:
        ...
        sess.run(trining_init_op, {file_name_train: "./data/amazon_train.ids"})
        train_loss = 0.0
        train_valid_words = 0
        saver = tf.train.Saver()
        while True:
        ...
        sess.run(validation_init_op, {file_name_validation: "./data/amazon_test.ids"})
        dev_loss = 0.0
        dev_valid_words = 0
        while True:
            try:
                ...
                if dev_ppl < best_score:
                    patience = 1
                    saver.save(sess, "model/amazon_full_best_model.ckpt")
                    best_score = dev_ppl
                else:
                    patience -= 1
...
#---------------------------------------------------------------------------------------
    ...
    time_consumed = 0
    max_acc = 0
    model_name = 'han_adv_pretrain_amazon_full'
    ...
    out_file = 'log_adv_vir_han_pretrain_amazon_full/'
