...
parser.add_argument('--sen', metavar='S', default=5, type=int,
...
def keras_process_sogou(MAX_DOCUMENT_LENGTH,MAX_WORD_LENGTH,MAX_VOCAB):
    ...
    data_test = pd.read_csv('../../sogou_news_csv/train_m.csv', sep=',')
...
with graph.as_default():
    ...
    training_dataset = tf.data.TextLineDataset(file_name_train).map(parse).padded_batch(lm_batch_size, padded_shapes=([45], [45]))
    print('training:',training_dataset.output_shapes)
    validation_dataset = tf.data.TextLineDataset(file_name_validation).map(parse).padded_batch(lm_batch_size, padded_shapes=([45], [45]))
...
#----------------------------------------------------------------------------------------
    ...
    while epoch < num_epochs:
        ...
        sess.run(trining_init_op, {file_name_train: "./data/AG_train.ids"})
        train_loss = 0.0
        train_valid_words = 0
        saver = tf.train.Saver()
        while True:
        ...
        sess.run(validation_init_op, {file_name_validation: "./data/AG_test.ids"})
        dev_loss = 0.0
        dev_valid_words = 0
        while True:
            try:
                ...
                if dev_ppl < best_score:
                    patience = 1
                    saver.save(sess, "model/best_model2.ckpt")
                    best_score = dev_ppl
                else:
                    patience -= 1
...
#---------------------------------------------------------------------------------------
    '''
    saver.restore(sess, "model/best_model2.ckpt")
    print("Start training")
    start = time.time()
    time_consumed = 0
    max_acc = 0
    model_name = 'han_adv_drp2_pretrain_'
    ...
    out_file = 'log_adv_vir_han_pretrain/'
